...
def process_candidates(candidate_path):
    """
    ...
    for txt_file in os.listdir(candidate_path):
        if txt_file.endswith('.txt'):
            with open(os.path.join(candidate_path, txt_file), 'r', encoding='utf-8') as f:
                text = f.read()
                lemmatized_text = text_processor(text)
                bigrams = generate_bigrams(lemmatized_text)
                candidate_corpus.extend(lemmatized_text)
                candidate_corpus.extend(bigrams)

    return candidate_corpus


def process_unknown(unknown_path, problem_folder, output_folder):
    """
    ...
    for txt_file in os.listdir(unknown_path):
        if txt_file.endswith('.txt'):
            with open(os.path.join(unknown_path, txt_file), 'r', encoding='utf-8') as f:
                ...
                lemmatized_text = text_processor(text)
                bigrams = generate_bigrams(lemmatized_text)
                log_frequencies = calculate_log_frequencies(lemmatized_text + bigrams)
...
if __name__ == "__main__":
    base_folder = r'D:\kod\term pro\pan19-cross-domain-authorship-attribution-training-dataset-2019-01-23'
    folders_to_process = ['problem00002', 'problem00004']
    output_folder = r'D:\kod\term pro\preprocessed_data'
    os.makedirs(output_folder, exist_ok=True)
